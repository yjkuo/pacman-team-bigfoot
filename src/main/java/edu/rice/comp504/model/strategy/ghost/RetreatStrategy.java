...
public class RetreatStrategy implements IUpdateGhostStrategy {
    private static IUpdateStrategy singleton;
    private int[][] layout;

    private RetreatStrategy(int[][] layout) {
        this.layout = layout;
    }
    ...
    public static IUpdateStrategy makeStrategy(int[][] layout) {
        if (singleton == null) {
            singleton = new RetreatStrategy(layout);
        }
    ...
    public void updateState(ACharacter pacman, ACharacter context) {
        ...
        if (pacman.getName().equals("pacman")) {
            ...
            if (dir <= 0 || context.detectCollisionWithWalls(dir - 1, layout)) {
